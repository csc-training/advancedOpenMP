...
!!       equal to the number of devices.
  !$omp parallel do num_threads(ndevs) private(istart,iend)
  do idev=0,ndevs-1
...
!!    print*, istart, iend
      ...
      !$omp target device(idev) map(tofrom: x(istart:iend) &
      !$omp&                                y(istart:iend)
      do i=istart,iend
         x(i) = x(i) + a*y(i);
      enddo
...
!!do i=1,N; if(x(i) /= 3)  stop("y != 3"); enddo
  do i=1,N; if(x(i) /= 3) print*,"failed"; enddo
